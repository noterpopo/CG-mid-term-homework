...
vec2 allRotationsLshape[19][4] =
							  {{vec2(0, 0), vec2(1, 0), vec2(1, -1), vec2(0, -1)}, //o
							   ...
							   {vec2(0, 1), vec2(1, 1), vec2(0, 0), vec2(0, -1)},
							   {vec2(-1, 1), vec2(-1, 0), vec2(0, 0), vec2(1, 0)},
							   {vec2(-1, -1), vec2(0, -1), vec2(0, 0), vec2(0, 1)},
							   {vec2(-1, 0), vec2(0, -1), vec2(0, 0), vec2(1, 0)},//t
							   {vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, -1)},
							   {vec2(-1, 0), vec2(0, 0), vec2(0, 1), vec2(1, 0)},
							   {vec2(-1, 0), vec2(0, 1), vec2(0, 0), vec2(0, -1)}
							   };
//辅助数组，用来表明各种类型方块的初始下标							   
int xb[7]={0,1,3,5,7,11,15};
...
void newtile()
{
	...
	{
		//初始化为选取种类的默认方向
		tile[i] = allRotationsLshape[xb[type]][i];
	}
	rotation = 0;
...
void rotate()
{
	...
	// 计算得到下一个旋转方向
	int nextrotation = (rotation + 1) % rotate_num;

	// 检查当前旋转之后的位置的有效性
	if (checkvalid((allRotationsLshape[xb[type] +nextrotation][0]) + tilepos)
		&& checkvalid((allRotationsLshape[xb[type] +nextrotation][1]) + tilepos)
		&& checkvalid((allRotationsLshape[xb[type] +nextrotation][2]) + tilepos)
		&& checkvalid((allRotationsLshape[xb[type] +nextrotation][3]) + tilepos))
	{
		// 更新旋转，将当前方块设置为旋转之后的方块
		rotation = nextrotation;
		for (int i = 0; i < 4; i++)
			tile[i] = allRotationsLshape[xb[type] +rotation][i];

		updatetile();
	}
}

void getpoint(int row)
{
	for(int i=0;i<10;++i)
	{
		for(int j=row;j<20-1;++j) //row上的均会下落
		{
			//对row上面的元素进行判断
			if(board[i][j+1]==true) //如果之前有方格，就修改颜色
			{
				changecellcolour(vec2(i,j), boardcolours[(int)(6*(10*(j+1) + i))]);
			}
			else       //否则直接设为空方格
			{
				board[i][j]=false;
...
void movedown(int sig)
{
	if (!movetile(vec2(0, -1))&&sig==0)
	{
		settile();
		newtile();
	}
	//定时执行movedown
	glutTimerFunc(1000,movedown,ispause);
}
